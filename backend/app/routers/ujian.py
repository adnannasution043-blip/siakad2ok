...
DOSEN_ROLES = ["super_admin", "admin_akademik", "staf", "dosen", "kaprodi"]
...
def create_jadwal(body: dict):
    ...
    # Cek duplikasi jadwal ujian kelas + jenis yang sama
    ...
    return {"success": True, "data": rec, "message": "Jadwal ujian berhasil dibuat", "meta": None}


@router.get("/jadwal/{jadwal_id}")
def get_jadwal(jadwal_id: str):
    j = find_by_id("jadwal_ujian", jadwal_id)
    if not j or j.get("deleted_at"):
        raise HTTPException(404, "Jadwal ujian tidak ditemukan")
    return {"success": True, "data": j, "message": "Berhasil", "meta": None}


@router.put("/jadwal/{jadwal_id}")
def update_jadwal(jadwal_id: str, body: dict):
    j = find_by_id("jadwal_ujian", jadwal_id)
    if not j or j.get("deleted_at"):
        raise HTTPException(404, "Jadwal ujian tidak ditemukan")
    ...
    return {"success": True, "data": j, "message": "Jadwal berhasil diperbarui", "meta": None}


@router.delete("/jadwal/{jadwal_id}")
def delete_jadwal(jadwal_id: str):
    j = find_by_id("jadwal_ujian", jadwal_id)
    if not j or j.get("deleted_at"):
        raise HTTPException(404, "Jadwal ujian tidak ditemukan")
    if j.get("status") == "selesai":
    ...
    return {"success": True, "data": None, "message": "Jadwal dihapus", "meta": None}
...
def generate_kartu(body: dict):
    ...
    return {
        "success": True,
        "data": {"dibuat": dibuat, "diperbarui": diperbarui},
        "message": f"Generate selesai: {dibuat} kartu baru, {diperbarui} diperbarui",
        "meta": None,
    }


@router.get("/kartu/{kartu_id}")
def get_kartu(kartu_id: str):
    k = find_by_id("kartu_ujian", kartu_id)
    if not k or k.get("deleted_at"):
        raise HTTPException(404, "Kartu ujian tidak ditemukan")

    # Enrich: tambahkan jadwal ujian per kelas
...
def create_berita_acara(body: dict):
    ...
    return {"success": True, "data": rec, "message": "Berita acara berhasil dibuat", "meta": None}


@router.get("/berita-acara/{ba_id}")
def get_berita_acara(ba_id: str):
    ba = find_by_id("berita_acara_ujian", ba_id)
    if not ba or ba.get("deleted_at"):
        raise HTTPException(404, "Berita acara tidak ditemukan")
    return {"success": True, "data": ba, "message": "Berhasil", "meta": None}


@router.put("/berita-acara/{ba_id}")
def update_berita_acara(ba_id: str, body: dict):
    ba = find_by_id("berita_acara_ujian", ba_id)
    if not ba or ba.get("deleted_at"):
        raise HTTPException(404, "Berita acara tidak ditemukan")
    ...
    return {"success": True, "data": ba, "message": "Berita acara berhasil diperbarui", "meta": None}


# ─── RINGKASAN / STATS ────────────────────────────────────────────────────────

@router.get("/stats")
def get_stats(semester: Optional[str] = None):
    jadwal = [j for j in read_all("jadwal_ujian") if not j.get("deleted_at")]
    kartu = [k for k in read_all("kartu_ujian") if not k.get("deleted_at")]
    ba = [b for b in read_all("berita_acara_ujian") if not b.get("deleted_at")]
