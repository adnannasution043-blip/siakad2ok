...
def create_program(body: dict):
    ...
    return {"success": True, "data": rec, "message": "Program SP berhasil dibuat", "meta": None}


@router.get("/{sp_id}")
def get_program(sp_id: str):
...
@router.put("/{sp_id}")
def update_program(sp_id: str, body: dict):
    p = find_by_id("program_sp", sp_id)
    if not p or p.get("deleted_at"):
        raise HTTPException(404, "Program SP tidak ditemukan")
    ...
    return {"success": True, "data": p, "message": "Program SP diperbarui", "meta": None}


@router.delete("/{sp_id}")
def delete_program(sp_id: str):
    p = find_by_id("program_sp", sp_id)
    if not p or p.get("deleted_at"):
        raise HTTPException(404, "Program SP tidak ditemukan")
    ...
    return {"success": True, "data": None, "message": "Program SP dihapus", "meta": None}
...
def create_kelas(sp_id: str, body: dict):
    sp = find_by_id("program_sp", sp_id)
    ...
    insert("kelas_sp", rec)
    return {"success": True, "data": rec, "message": "Kelas SP berhasil dibuat", "meta": None}


@router.put("/kelas/{kelas_id}")
def update_kelas(kelas_id: str, body: dict):
    ...
    return {"success": True, "data": k, "message": "Kelas SP diperbarui", "meta": None}
...
def delete_kelas(kelas_id: str):
    ...
    return {"success": True, "data": None, "message": "Kelas SP dihapus", "meta": None}
...
def daftar_peserta(kelas_id: str, body: dict):
    ...
    # Validasi nilai lama: harus ada nilai D, E, atau C untuk MK ini
    ...
    if not nilai_asal:
        raise HTTPException(400, f"Mahasiswa tidak memiliki nilai D/E/C di MK {kls['mata_kuliah_nama']} — tidak berhak mengambil SP")
    ...
    # Update jumlah_peserta di kelas
    kls["jumlah_peserta"] = kls.get("jumlah_peserta", 0) + 1
    update("kelas_sp", kelas_id, kls)

    return {"success": True, "data": rec, "message": "Berhasil mendaftar SP", "meta": None}


@router.delete("/peserta/{peserta_id}")
def hapus_peserta(peserta_id: str):
    ...
    return {"success": True, "data": None, "message": "Peserta dihapus", "meta": None}


@router.post("/peserta/{peserta_id}/nilai")
def input_nilai(peserta_id: str, body: dict):
    ps = find_by_id("peserta_sp", peserta_id)
    if not ps or ps.get("deleted_at"):
        raise HTTPException(404, "Data peserta tidak ditemukan")
    ...
    # Aturan: nilai SP replace jika asal D/E, ambil terbaik jika asal C/C+
    nilai_asal_huruf = ps.get("nilai_asal", "")
    if nilai_asal_huruf in ("C", "C+"):
        # Ambil terbaik — kalau SP lebih buruk dari C, tetap pakai nilai asal
    ...
    return {"success": True, "data": ps, "message": "Nilai SP berhasil diinput", "meta": None}
