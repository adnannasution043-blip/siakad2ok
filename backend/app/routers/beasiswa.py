...
def get_stats(authorization: str = Header(default="dev")):
    ...
    if user.get("role") == "mahasiswa":
        ...
        daftar = [d for d in daftar if d.get("mahasiswa_id") == mhs_id]
...
@router.post("")
def create_beasiswa(body: dict, authorization: str = Header(default="dev")):
    user = get_user_from_request(authorization)
    ...
    insert("beasiswa", rec)
    return ok(rec, "Program beasiswa berhasil dibuat")
...
def list_pendaftar(
    ...
    if user.get("role") == "mahasiswa":
        mhs = get_mahasiswa_for_user(user)
        mhs_id = mhs["id"] if mhs else ""
        data = [d for d in data if d.get("mahasiswa_id") == mhs_id]
...
@router.get("/{bsw_id}")
def get_beasiswa(bsw_id: str, authorization: str = Header(default="dev")):
    get_user_from_request(authorization)
...
@router.put("/{bsw_id}")
def update_beasiswa(bsw_id: str, body: dict, authorization: str = Header(default="dev")):
    user = get_user_from_request(authorization)
    check_role(user, ADMIN)
    b = find_by_id("beasiswa", bsw_id)
    if not b or b.get("deleted_at"):
    ...
    return ok(b, "Program beasiswa diperbarui")


@router.delete("/{bsw_id}")
def delete_beasiswa(bsw_id: str, authorization: str = Header(default="dev")):
    user = get_user_from_request(authorization)
    check_role(user, ADMIN)
    b = find_by_id("beasiswa", bsw_id)
    if not b or b.get("deleted_at"):
    ...
    return ok(None, "Program beasiswa dihapus")


# ─── PENDAFTARAN ──────────────────────────────────────────────────────────────

@router.post("/{bsw_id}/daftar")
def daftar_beasiswa(bsw_id: str, body: dict, authorization: str = Header(default="dev")):
    user = get_user_from_request(authorization)
    ...
    return ok(rec, "Pendaftaran beasiswa berhasil")


@router.post("/daftar/{daftar_id}/setujui")
def setujui_pendaftaran(daftar_id: str, authorization: str = Header(default="dev")):
    user = get_user_from_request(authorization)
    check_role(user, ADMIN)
    ...
    return ok(d, "Pendaftaran diterima")


@router.post("/daftar/{daftar_id}/tolak")
def tolak_pendaftaran(daftar_id: str, body: dict, authorization: str = Header(default="dev")):
    user = get_user_from_request(authorization)
    check_role(user, ADMIN)
    d = find_by_id("daftar_beasiswa", daftar_id)
    if not d or d.get("deleted_at"):
    ...
    d["tgl_keputusan"] = _now()
    update("daftar_beasiswa", daftar_id, d)
    return ok(d, "Pendaftaran ditolak")
