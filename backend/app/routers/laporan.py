...
def laporan_mahasiswa(tahun_akademik: Optional[str] = None):
    mahasiswa = _active("mahasiswa")
...
def laporan_akademik():
    ...
    # Nilai distribution by nilai_akhir ranges
    grade_dist = {"A (85-100)": 0, "B+ (80-84)": 0, "B (75-79)": 0,
                  "C+ (70-74)": 0, "C (65-69)": 0, "D (55-64)": 0, "E (<55)": 0}
    for n in nilai_list:
        v = float(n.get("nilai_akhir") or 0)
        if v >= 85:
            grade_dist["A (85-100)"] += 1
        elif v >= 80:
            grade_dist["B+ (80-84)"] += 1
        elif v >= 75:
            grade_dist["B (75-79)"] += 1
        elif v >= 70:
            grade_dist["C+ (70-74)"] += 1
        elif v >= 65:
            grade_dist["C (65-69)"] += 1
        elif v >= 55:
            grade_dist["D (55-64)"] += 1
        else:
            grade_dist["E (<55)"] += 1
    ...
    return {
        ...
        "data": {
            "nilai": {
                ...
                "rata_rata": avg_nilai,
                "distribusi": grade_dist,
            },
            "krs": {
                "total": len(krs_list),
                "by_status": by_status_krs,
                "total_sks_disetujui": total_sks_krs,
            },
            "kelas": {
                "total": len(kelas_list),
                "data": kelas_data,
            },
        },
        "message": "OK"
...
def laporan_riset():
    ...
    by_status_pen = _count_by(penelitian, "status")
    by_skema_pen = _count_by(penelitian, "skema")
    by_status_pkm = _count_by(pkm, "status")
    by_skema_pkm = _count_by(pkm, "skema")

    by_jenis_luaran_pen = _count_by(luaran_pen, "jenis")
    by_jenis_luaran_pkm = _count_by(luaran_pkm, "jenis")

    by_validasi_pen = _count_by(luaran_pen, "status_validasi")

    total_dana_pen = sum(p.get("dana", 0) or 0 for p in penelitian)
    total_dana_pkm = sum(p.get("dana", 0) or 0 for p in pkm)
    ...
    return {
        ...
        "data": {
            "penelitian": {
                ...
                "by_status": by_status_pen,
                "by_skema": by_skema_pen,
                "total_dana": total_dana_pen,
                "luaran": len(luaran_pen),
                "luaran_by_jenis": by_jenis_luaran_pen,
                "luaran_by_validasi": by_validasi_pen,
            },
            "pkm": {
                "total": len(pkm),
                "by_status": by_status_pkm,
                "by_skema": by_skema_pkm,
                "total_dana": total_dana_pkm,
                "luaran": len(luaran_pkm),
                "luaran_by_jenis": by_jenis_luaran_pkm,
            },
        },
        "message": "OK"
    }
...
def laporan_sdm():
    ...
    by_jabatan_dosen = _count_by(dosen, "jabatan_fungsional")
    by_pendidikan_dosen = _count_by(dosen, "pendidikan_terakhir")
    by_status_pegawai = _count_by(pegawai, "status")
    by_unit_pegawai = _count_by(pegawai, "unit_kerja")
    ...
    return {
        ...
        "data": {
            "dosen": {
                "total": len(dosen),
                "by_jabatan": by_jabatan_dosen,
                "by_pendidikan": by_pendidikan_dosen,
            },
            "pegawai": {
                "total": len(pegawai),
                "by_status": by_status_pegawai,
                "by_unit": by_unit_pegawai,
                "total_gaji_per_bulan": total_gaji,
            },
            "akreditasi": {
                "total_prodi": len(akreditasi_prodi),
                "by_peringkat": by_peringkat,
...
def laporan_kemahasiswaan():
    ...
    by_status_ta = _count_by(ta, "status")
    by_status_magang = _count_by(magang, "status")
    by_status_beasiswa = _count_by(beasiswa, "status")

    # PMB by gelombang
    by_gelombang_pmb = _count_by(pmb, "gelombang_id")
    pmb_diterima = len([p for p in pmb if p.get("status") == "diterima"])

    # Surat by jenis
    by_jenis_surat = _count_by(surat, "jenis_surat")
    by_status_surat = _count_by(surat, "status")

    # Alumni pekerjaan
    alumni_bekerja = len([a for a in alumni if a.get("instansi")])
    alumni_belum = len(alumni) - alumni_bekerja
    ...
    return {
        ...
        "data": {
            "ta": {
                "total": len(ta),
                "by_status": by_status_ta,
            },
            "magang": {
                "total": len(magang),
                "by_status": by_status_magang,
            },
            "beasiswa": {
                "total_pendaftar": len(beasiswa),
                "by_status": by_status_beasiswa,
            },
            "ukm": {
                "total_anggota": len(ukm_anggota),
            },
            "alumni": {
                "total": len(alumni),
                "sudah_bekerja": alumni_bekerja,
                "belum_bekerja": alumni_belum,
                "rasio_bekerja": round(alumni_bekerja / len(alumni) * 100, 1) if alumni else 0,
            },
            "pmb": {
                "total_pendaftar": len(pmb),
                "diterima": pmb_diterima,
                "by_gelombang": by_gelombang_pmb,
            },
            "surat": {
                "total": len(surat),
                "by_jenis": by_jenis_surat,
                "by_status": by_status_surat,
            },
        },
        "message": "OK"
    }
