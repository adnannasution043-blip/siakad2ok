...
def get_stats():
    ...
    terlambat = len([p for p in pinjam if p.get("status") == "dipinjam" and p.get("tgl_kembali_estimasi", "9999") < today])
...
def create_koleksi(body: dict):
    ...
    return {"success": True, "data": record, "message": "Koleksi berhasil ditambahkan"}

@router.get("/koleksi/{buku_id}")
def get_koleksi(buku_id: str):
...
def update_koleksi(buku_id: str, body: dict):
    rec = find_by_id("koleksi_perpustakaan", buku_id)
    if not rec or rec.get("deleted_at"):
        raise HTTPException(404, "Buku tidak ditemukan")
    updatable = ["isbn", "judul", "penulis", "penerbit", "tahun", "kategori", "stok_total", "stok_tersedia", "lokasi_rak"]
    changes = {k: body[k] for k in updatable if k in body}
    updated = update("koleksi_perpustakaan", buku_id, changes)
    return {"success": True, "data": updated, "message": "Koleksi berhasil diperbarui"}

@router.delete("/koleksi/{buku_id}")
def delete_koleksi(buku_id: str):
    rec = find_by_id("koleksi_perpustakaan", buku_id)
    if not rec or rec.get("deleted_at"):
        raise HTTPException(404, "Buku tidak ditemukan")
    soft_delete("koleksi_perpustakaan", buku_id)
    return {"success": True, "data": None, "message": "Koleksi berhasil dihapus"}
...
def create_peminjaman(body: dict):
    ...
    # Kurangi stok
    update("koleksi_perpustakaan", body["buku_id"], {"stok_tersedia": max(0, (buku.get("stok_tersedia") or 1) - 1)})

    return {"success": True, "data": record, "message": "Peminjaman berhasil dicatat"}

@router.post("/peminjaman/{pinjam_id}/kembali")
def kembalikan_buku(pinjam_id: str, body: dict = None):
    ...
    if buku and not buku.get("deleted_at"):
        update("koleksi_perpustakaan", buku["id"], {"stok_tersedia": (buku.get("stok_tersedia") or 0) + 1})

    msg = f"Buku berhasil dikembalikan"
    if denda:
